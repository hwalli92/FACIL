...
class ExemplarsDataset(NTUDataset):
    """Exemplar storage for approaches with an interface of Dataset"""

    def __init__(self, transform, class_indices, dataset_args,
                 num_exemplars=0, num_exemplars_per_class=0, exemplar_selection='random'):
        super().__init__({'x': [], 'y': []}, transform, class_indices=class_indices, **dataset_args)
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform):
        if self._is_active():
            self.data, self.labels = self.exemplars_selector(model, trn_loader, selection_transform)
